...
export default function WhyUs() {
  ...
  return (
    <section className="py-24 2xl:py-32 px-6 relative overflow-hidden">
      ...
      {/* Decorative elements */}
      <div className="absolute top-20 left-10 w-64 h-64 2xl:w-96 2xl:h-96 bg-accent-purple/10 rounded-full blur-3xl"></div>
      <div className="absolute bottom-20 right-10 w-64 h-64 2xl:w-96 2xl:h-96 bg-accent-pink/10 rounded-full blur-3xl"></div>
      ...
      <div className="relative z-10 max-w-7xl 2xl:max-w-[1400px] mx-auto">
        <div className="text-center mb-12 md:mb-16">
          ...
          <p className="text-base md:text-xl 2xl:text-2xl text-text-secondary max-w-2xl mx-auto">
            {t.whyUs.subtitle}
          </p>
        </div>
        ...
        <div className="hidden md:grid md:grid-cols-3 gap-8 2xl:gap-10">
          {t.whyUs.features.map((feature, index) => (
            ...
            >
              {/* Card */}
              <div className="relative h-full p-8 2xl:p-10 rounded-2xl border border-white/10 bg-gradient-to-br from-bg-darker/80 to-bg-dark/80 backdrop-blur-sm hover:border-accent-lime/50 transition-all duration-300">
                {/* Title with hover scale */}
                <h3 className="text-2xl 2xl:text-3xl font-bold mb-4 gradient-text-lime-gold transform group-hover:scale-110 transition-transform duration-300 origin-left whitespace-pre-line">
                  {feature.title}
                </h3>
        ...
        <div className="text-center mt-10 md:mt-12">
          <Link
            href="/about"
            className="inline-block px-6 py-3 md:px-8 md:py-4 2xl:px-10 2xl:py-5 border border-accent-lime rounded-lg text-text-primary font-bold uppercase text-sm md:text-base 2xl:text-lg hover:bg-accent-lime/20 hover:scale-105 hover:glow-lime transition-all duration-300"
          >
            {t.whyUs.aboutBtn}
          </Link>
        </div>
