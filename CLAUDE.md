...
4. `git push` — пуш в main (триггерит деплой на Vercel)
...
- **Реализация:** Клиентская отправка напрямую в Telegram Bot API из браузера (файл `app/get-concept/page.tsx`). Серверный API-роут `/api/send-concept` **не используется** — static export не поддерживает API-роуты.
...
│   └── i18n/                # Переводы RU/EN
├── public/
│   ├── sitemap.xml          # XML-карта сайта (с lastmod датами)
│   ├── robots.txt           # Правила для ботов (AI-краулеры разрешены)
│   ├── og-image.jpg         # OpenGraph изображение (1200x630)
│   ├── llms.txt             # Контекст для AI-поисковиков (краткий)
│   ├── llms-full.txt        # Контекст для AI-поисковиков (полный)
...
### Критические SEO-фиксы (11 февраля 2026)

**Проблема:** Google не индексировал сайт ВООБЩЕ (site:3dgryz.ru = 0 результатов). Яндекс — 16 страниц, но 0 кликов.

**Исправлено:**
1. `robots.txt` — убрано `Disallow: /_next/` (блокировало Googlebot от загрузки CSS/JS)
2. `robots.txt` — убран `Crawl-delay: 1` для всех ботов (замедлял индексацию нового сайта)
3. `layout.tsx` — удалён дублирующий `<meta name="robots">` тег (конфликтовал с metadata.robots)
4. `layout.tsx` — удалён сломанный hreflang `/en/` (страница не существует, Google фиксировал ошибку)
5. `layout.tsx` — snippet-директивы перенесены в metadata.robots (max-snippet, max-image-preview, max-video-preview)
6. Исправлены дублирующиеся title "| 3D.GRYZ | 3D.GRYZ" на portfolio, ai-content, blog
7. Улучшены title и description для повышения CTR
8. `sitemap.xml` — добавлены `<lastmod>` даты ко всем 18 URL
9. Созданы `llms.txt` и `llms-full.txt` для AI-поисковиков (Perplexity, Claude, ChatGPT)

**Дополнительно исправлено (11 февраля 2026):**
10. `og-image.jpg` — создан (1200x630, тёмный фон + лого + текст, 35 КБ)
11. Удалён `.github/workflows/deploy.yml` (GitHub Actions деплой — не нужен, сайт на Vercel)
12. Удалён `public/CNAME` (артефакт GitHub Pages)

**Google Search Console (11 февраля 2026):**
- Sitemap отправлен через API
- 8 из 16 страниц проиндексированы Google
...
### Generative SEO (AI-поисковики)

**Настроено (техника 9/10):**
- 11 типов JSON-LD схем (Organization, WebSite, ProfessionalService, Speakable, FAQPage, Article, Service, CreativeWork, HowTo, VideoObject, Breadcrumb)
- 8 AI-краулеров разрешены в robots.txt (GPTBot, ChatGPT-User, Claude, PerplexityBot, Google-Extended, YandexMedianaBot, Bytespider, CCBot)
- SpeakableSpecification на главной
- FAQ Schema на всех 9 блог-постах + главной + услугах (~50 Q&A)
- RSS Feed /feed.xml
- data-speakable атрибуты
- max-snippet:-1 (неограниченные сниппеты)
- llms.txt + llms-full.txt (добавлено 11.02.2026)

**Результат пока 1/10:** AI-поисковики не цитируют сайт т.к. нет Google-индексации и бэклинков.
